...
%}
orig = ones(size(vert0,1),3).*orig;
dir = ones(size(vert0,1),3).*dir;
...
if nVarargs>0 && isstruct(varargin{1})
  ...
  if (isfield(options, 'fullReturn'  )), fullReturn   = options.fullReturn;   end
else
  while (k<=nVarargs)
...
pvec  = crossMex(dir, edge2,2);  % begin calculating determinant - also used to calculate U parameter
...
if fullReturn
  ...
  qvec = crossMex(tvec, edge1,2);    % prepare to test V parameter
  ...
  qvec = crossMex(tvec(ok,:), edge1(ok,:),2); % prepare to test V parameter
